...
#    def _is_descended_from(store, block_id, ancestor_id):
    ...
    def import_tx(store, tx, is_coinbase, chain):
        ...
        store.sql("""
            INSERT INTO tx (tx_id, tx_hash, tx_version, tx_lockTime, tx_extra_payload, tx_size)
            VALUES (?, ?, ?, ?, ?, ?)""",
    ...
    def export_tx(store, tx_id=None, tx_hash=None, decimals=8, format="api", chain=None):
        ...
        if tx_id is not None:
            row = store.selectrow("""
                SELECT tx_hash, tx_version, tx_lockTime, tx_extra_payload, tx_size
                  FROM tx
                 WHERE tx_id = ?
            """, (tx_id,))
            if row is None:
        ...
        tx['extra_payload']                       = store.binout_hex(row[3])
        tx['size']                                = int(row[4])
    ...
    def _export_tx_detail(store, tx_hash, chain):
        ...
        tx = {
            ...
            'type': int(row[1]) >> 16,
            'lockTime': int(row[2]),
            'extra_payload': store.binout_hex(row[3]),
            'size': int(row[4]),
            }
